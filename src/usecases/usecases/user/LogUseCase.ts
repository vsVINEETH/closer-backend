    ...
    async loginAuth(name: string, email: string): Promise<{
        ...
        try {
            ...
            if (!user) {
                ...
                const createdUser = await this.userRepository.createAuthUser(newUser);
                const tokens = this.generateToken(createdUser.id, this.role);
                if(createdUser?.image){
                    createdUser.image = await Promise.all(
                        createdUser.image.map(async (val) => await this.s3.retrieveFromS3(val as string))
                    );
                };
                return {
                    user: {
